...
void TextScreen::setSplitScreen(uint16_t scanline)
{
    if(scanline < m_height) {
...
uint8_t TextScreen::getHPanning()
{
    uint8_t hpan = 0;
    uint16_t isra = 0x03da;
    if(m_crtc_addr == CRTC_ADDR_MONO) {
    ...
    ACR_IN(isra, ACR_HPELPAN, hpan);
    return hpan;
}

uint8_t TextScreen::getVPanning()
{
    uint8_t prs;
    CRTC_IN(m_crtc_addr, CRTC_PRESET_ROW_SCAN, prs);
    return (prs & 0x1f);
...
void TextScreen::setMode(int bios, int cols, int rows, int boxw, int boxh)
{
    ...
    if(rows == 25) {
        setBIOSFont(1, boxh);
        snprintf(buf, 81, "Mode %d %dx%d %dx%d %dx%d", bios, cols, rows, boxw, boxh, m_width, m_height);
    } else {
        setBIOSFont(0, boxh, false);
        setBIOSFont(1, boxh, true);
        snprintf(buf, 81, "%dx%d %dx%d %dx%d", cols, rows, boxw, boxh, m_width, m_height);
    }
